...
export class ProductQueryService {
  ...
  private apiUrl2 = environment.apihost + 'read_mycollection_bylang/' ;
  ...
  getProducts_bylang ( lang : string): Observable<Product[]> {
    return this.http.get<Product[]>( this.apiUrl2 + lang + '/', { headers: new HttpHeaders({'Content-Type':  'application/json'}) } )
      .pipe(
        tap(_ => this.log('fetched products by language')),
        catchError(this.handleError('getProducts by language', []))
      );
  }
