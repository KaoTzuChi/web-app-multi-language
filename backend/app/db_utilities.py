...
class db_util:
    def __init__(self, param_collection, param_language):
        ...
        if (self.host!=None) & (self.dbname!=None) & (self.user!=None) & (self.password!=None):
            ...
            self.set_language(param_language)
    ...
    def read_documents_bylang(self, checkfield):
        try:
            _db = self.get_database()
            _colname = self.get_collection_name()
            _lang = self.get_language()
            _cursor = None
            if _db:
                if _colname:
                    _collection = _db[_colname]
                    if _collection:
                        _field = '%s.%s' % (checkfield, _lang)
                        _cursor = _collection.find({_field: {'$exists': True}})
            return _cursor     
        except Exception as e:
            print('fail >>> %s.%s: %s' % ('db_util','read_documents_bylang ex=',e))
